...
\abstract{We propose that gravity can be understood as the physical backreaction of quantum measurement. Standard results, horizon scales, entropy bounds, and the Bekenstein--Hawking area law, follow from Landauer's principle at the Unruh temperature, from which tempurature drops out entirely. To ground this, we examine a specific example from previous work, a bilocal microstate construction in which entangled photon pairs, emitted from a localised source, propagate into opposite Rindler wedges where polarisation measurements produce discrete energy-momentum impulses and correlated classical bits. Each microstate is described geometrically by an Aichelburg--Sexl pp-wave solution, whose longitudinal shift generates spatial separation between the wedges and whose transverse focusing increments the horizon area. The framework provides an operational realisation of ER=EPR at the level of individual microstates, and complements the thermodynamic approaches of Jacobson and Verlinde by supplying the microphysical process underlying each entropy increment.}
...
We take the interaction time to be of order the light-crossing time across the interaction region, $\Delta t \sim 2r/c$, corresponding to a causal exchange over the scale $r$. The numerical factor is not fixed a priori at this level; it will be determined by requiring consistency with the gravitational scale that emerges below. For a detector of mass $m_d$, Newton's second law in impulse form gives
\begin{equation}
...
This step should be understood as a calibration: we match the acceleration inferred from the measurement-induced impulse to the Newtonian gravitational field in order to identify the characteristic length scale associated with the detector. The thrust quantum carries energy $\Delta E$ and produces an acceleration $a$ on the detector. Identifying this with the Newtonian gravitational acceleration due to a mass $m_t$ at distance $r$.
\begin{equation}
a = \frac{G m_t}{r^2},
\end{equation}
